    ...
    foundParameters.forEach(function addFoundParameter(parameterName) {
        ...
        if (isInCommon) {
            console.warn(
                "Parameter name '" +
                    parameterName +
                    "' was specified using in the metadata.common.parameters.path array, however it was never added to the state by calling require('swagger-spec-express').common.parameters.addPath(...). Will construct a basic parameter"
            );
        }
