...
function initialise(app, options, callback) {
    ...
    state.expressRoutes = app._router.stack;
...
function addHeaderParameter(header) {
    ...
    header.in = 'header';
    state.common.tags[header.name] = header;
}
...
function addBodyParameter(body) {
    ...
    body.in = 'body';
    state.common.tags[body.name] = body;
}
...
function addQueryParameter(query) {
    ...
    query.in = 'query';
    state.common.tags[query.name] = query;
}
...
function addFormDataParameter(formData) {
    ...
    state.common.tags[formData.name] = formData;
}
...
function addPathParameter(path) {
    ...
    path.in = 'path';
    state.common.tags[path.name] = path;
}
...
function describeRoute(router, metaData) {
    var lastRoute = router.stack[router.stack.length - 1];
    if (!lastRoute.route) {
        return;
    }
    if (!Object.keys(lastRoute.route.methods)[0]) {
        return;
    }
    metaData.path = lastRoute.route.path;
    metaData.verb = Object.keys(lastRoute.route.methods)[0];
    lastRoute.route.swaggerData = metaData;
}

function describe(metaData) {
    var lastRoute = state.expressRoutes[state.expressRoutes.length - 1];
    if (!lastRoute.route) {
        return;
    }
    if (!Object.keys(lastRoute.route.methods)[0]) {
    ...
    addMetaDataToRoute(lastRoute.route.path, verb, metaData);
}

function addMetaDataToRoute(path, verb, metaData) {
    var route = null;
    metaData.path = path;
    metaData.verb = verb;
    for (var i = 0; i < state.expressRoutes.length; i++) {
        if (state.expressRoutes[i].route && state.expressRoutes[i].route.path === path) {
            route = state.expressRoutes[i].route;
            if (Object.keys(route.methods)[0] === verb) {
                route.swaggerData = metaData;
                break;
            }
        }
    }
