...
function addHeaderToResponse(response, stateCommon) {
    ...
    response.commonHeaders.forEach(function (headerName) {
        ...
        if (!foundHeader) {
            throw new Error(
                'Common header by the name of ' + headerName + ' could not be found for response : ' + JSON.stringify(response)
            );
        }
...
function addCommonParametersOfType(parameters, parameterTypeKey, stateCommon, operation) {
    ...
    if (!stateCommon.parameters) {
        throw new Error(
            "Can't specify common parameters on a route if you have not added any common parameters. Parameters : " +
...
function replaceModelInParameter(parameterType, parameter) {
    if (parameter.model || parameter.arrayOfModel) {
        ...
        if (parameterType !== 'body') {
            throw new Error(
                "Can't specify parameter.model or parameter.arrayOfModel if the parameter is not a body parameter. " +
